...
export const AntiGravityCanvas: React.FC = () => {
  ...
  const animate = useCallback((time: number) => {
    ...
    // Pulsating radial glow (indigo instead of blue)
    const centerX = canvas.width / 2
    const centerY = canvas.height / 2
    const pulseOpacity = Math.sin(time * 0.0008) * 0.035 + 0.07
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    mouseRef.current = { x: touch.clientX - rect.left, y: touch.clientY - rect.top, isActive: true }
  }

  const handleTouchEnd = () => { mouseRef.current.isActive = false }
